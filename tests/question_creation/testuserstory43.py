...
question_bank = [
    ...
    },
    {
        ...
        "options": [
            "Model too fitted to training data",
            "Lack of data",
            "Too many layers",
            "None"
        ],
...
# 3. Calculate performance per tag
...
if __name__ == "__main__":
    ...
    if not quiz_questions:
        ...
        for i, q in enumerate(quiz_questions, 1):
            print(f"\nQuestion {i}: {q['question']} (Tags: {q['tags']})")
            for idx, opt in enumerate(q["options"]):
                print(f"  {idx}. {opt}")

        # Simulate student answers for demonstration
        print("\nEnter student answers as the OPTION NUMBER (0,1,2,3):")
        student_answers = []
        for q in quiz_questions:
            while True:
                try:
                    ans_str = input(f"Answer for '{q['question']}': ").strip()
                    ans = int(ans_str)
                    ...
                    print("Please enter a NUMBER (0,1,2,3), not text.")
        ...
        # Save template (selected tags) as an example of "saving tag settings"
